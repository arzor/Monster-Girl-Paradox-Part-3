...
module BattleManager
  class << self
    ...
    def giveup=(other)
      ...
      return if $game_actors.nil? || !$game_actors.exist?(NWConst::Actor::LUCA)

      $game_actors[NWConst::Actor::LUCA].refresh
    end
    ...
    def process_abort
      SceneManager.scene.process_event_on_battle_end(:abort)
      replay_bgm_and_bgs
      SceneManager.return
      battle_end(1)
      true
    ...
    def process_defeat
      @phase = nil
      SceneManager.scene.process_event_on_battle_end(:defeat)
    ...
    def process_follow_ok(follower_name = nil)
      ...
      if $game_party.multi_party?
        $game_message.add("#{follower_name} heads back to the pocket castle!")
    ...
    def make_action_orders
      ...
      @fastest_actions = [@action_battlers.first].compact
      @action_battlers += ab.select { |m| m.actions.empty? }.shuffle
      return if @fastest_actions.empty?
...
module Cache
  class << self
    ...
    def check_filesize
      clear if @cache.size > 100
    end

    def try_create_bitmap(*args)
      return create_large_bitmap(args[0], args[1]) if args.size == 2 && args[0] * args[1] > 4_194_304
...
module DataManager
  class << self
    ...
    def make_thumbnailname(index)
      if index.is_a?(Integer)
        format("Save/Save%02d.png", index + 1)
      else
        format("Save/AutoSave%s.png", index)
      end
    end
    ...
    def get_thumbnail(index)
      @thumbnails[index] ||= create_thumbnail(index)
    end
    ...
    def load_normal_database
      ...
      $data_system.skill_types[NWConst::Ability::MASTER_ABILITY_STYPE] = NWConst::Ability::MASTER_ABILIRY
    end
    ...
    def auto_save_file_exists?
      ...
      header = load_header("01")
      return false if header && header.is_a?(Hash) && !header[:save_index].is_a?(Integer)
    ...
    def auto_save_game_without_rescue(index)
      ...
      File.rename(make_temp_filename, fname)
      true
    end
    ...
    def save_system_without_rescue
      ...
      File.rename(make_temp_filename, SaveSystemData.save_filename)
      true
    end
    ...
    def nw_analyze_database
      ...
      $data_common_events.compact.each do |ev|
        ev.list.shift while ev.list.first && ev.list.first.code == 999
    ...
    def create_thumbnail(index)
      ...
      return Bitmap.new(make_thumbnailname(index)) if File.exist?(make_thumbnailname(index))
      if File.exist?(make_filename(index)) && NWFileTest.system_exist?("NotThumbnailFile.png")
        return Bitmap.new("Graphics/System/NotThumbnailFile.png")
      end

      d = Bitmap.new(160, 120)
      d.stretch_blt(d.rect, @dummy_thumbnail, @dummy_thumbnail.rect)
      d
    end
...
module FavoriteItem
  ...
  def process_set_favorite_item
    Input.update
    return Sound.play_buzzer if favorite_window.item.nil?

    Sound.play_ok
    favorite_window.set_favorite_item
    favorite_window.refresh
    favorite_window.select([favorite_window.index - 1, 0].max) if favorite_window.favorite_mode
  ...
  def process_change_favorite_mode
    Input.update
    Sound.play_ok
    last_item = favorite_window.item
    favorite_window.change_favorite_mode
    favorite_window.refresh
    if favorite_window.active
      favorite_window.select_item(last_item)
    else
      favorite_window.select(-1)
    end
  end
  ...
  def update
    ...
    favorite_window.change_favorite_mode if off_favorite_mode? and @item_window.favorite_mode
    super
  end
...
module Help
  class << self
    ...
    def skill_type_key
      t = ""
      t += "\\C[0]"
      t += "#{Vocab.key_a}:Hide in battle + Disable in automatic combat"
      t += "(Disabled)" unless $game_system.conf[:bt_stype]
      t += "\n"
      t += "\\C[16]" if Input.press?(:X)
      t += "#{Vocab.key_x}＋↑/↓:Rearrange skillset order"
...
module ShowKey_Help
  class << self
    ...
    def favorite_item(state)
      "#{Vocab.key_y}:#{favorite(state)}"
    end

    def favorite_actor(state)
      "#{Vocab.key_a}:#{favorite(state)}"
    end
    ...
    def ability_form_call
      "#{Vocab.key_z}:Ability Set Function"
    end

    def ability_form_show
      "#{Vocab.key_x}:Ability Set Details"
    end
    #--------------------------------------------------------------------------
    # ● ショップ画面の装備品情報変更テキスト
    #--------------------------------------------------------------------------
...
module Vocab
  ...
  AbilitySaveMessage = "Which slot do you want to save the ability set to?"
  AbilityLoadMessage = "Which ability set do you want to load?"
  AbilitySave = "Save Ability Set"
  AbilityLoad = "Load Ability Set"
  ...
  class << self
    ...
    def params_a(param_id)
      ["ATK", "DEF", "MAG", "WIL", "AGI", "DEX","HP","MP"][param_id]
    end
    ...
    def etype(etype_id)
      ($data_system.terms.etypes + ["Attribute Accessory", "General Accessory"])[etype_id]
    end
